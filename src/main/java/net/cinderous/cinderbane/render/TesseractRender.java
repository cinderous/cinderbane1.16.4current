...
public class TesseractRender extends TileEntityRenderer<CinderousTesseractTileEntity> {

    public static final float TANK_THICKNESS = 0.3f;
    public static final float TANK_HEIGHT = 0f;
    public static final float TANK_BOTTOM = 0.0f;

    public TesseractRender(TileEntityRendererDispatcher rendererDispatcherIn) {
    ...
    @Override
    public void render(CinderousTesseractTileEntity tileEntityIn, float partialTicks, MatrixStack matrixStackIn,
        ...
        float fluidAmount = fluid.getAmount();
        float fluidCapacity = tileEntityIn.getTank().getCapacity();
        float scale = fluidAmount / fluidCapacity;
        //float scale = tileEntityIn.getTank().getCapacity();
        //float scale = fluid.getAmount() / (tileEntityIn.getTank().getCapacity());
        //Cinderbane.LOGGER.info(fluid.getAmount() / (tileEntityIn.getTank().getCapacity()) + "=" + fluid.getAmount() + "/" + (tileEntityIn.getTank().getCapacity()) );
        Cinderbane.LOGGER.info(scale);
